...
makeCacheMatrix <- function(x=numeric(),r,c) {                            ## x is a atomic vector, r=rows, c=columns
  ...
  setIM<-function(m) IM<<-m                                               ## Setting value in global variable
  list(getIM=getIM,getdata=getdata,setIM=setIM)                           ## Returning a list of functions
}
...
cacheSolve <- function(x,...) {         
  ...
  if (sum(as.logical(IM))!=0) {                                           ## Checking if cache is empty
    message("Getting Inverse from Cache")
    return(IM)                                                            ## Returning cached data
  }
  m<-x$getdata()                                                          ## Getting/Reading data in object environment
  IM<-x$setIM(solve(m))                                                   ## Returning inverse of matrix  
}